...
LimbPtr Body::getLimb(value_object::MuscleGroupType muscle_group) const noexcept {
...
MuscleGroupPtr Body::getMuscleGroup(value_object::MuscleGroupType muscle_group) const noexcept {
    if(LimbPtr ptr = getLimb(muscle_group))    
        return ptr->getMuscleGroup(muscle_group);
    return nullptr;
}

MuscleGroupPtr Body::getMuscleGroup(value_object::MuscleType muscle) const noexcept {
