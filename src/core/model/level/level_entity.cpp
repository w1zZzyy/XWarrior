...
Level::Level(value_object::LevelValue val, value_object::LevelType type,
             value_object::Experience req_exp) noexcept
    : val_(val), type_(type), required_exp_(req_exp) {}
...
std::optional<Level> Level::Build::build() const noexcept {
  ...
  return Level(val_.value(), type_.value(), exp_.value());
}
